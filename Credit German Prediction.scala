...
val creditDf= spark.read.format("csv")
    ...
    .schema(schema)
    .load("dbfs:/FileStore/tables/credit.csv")
    .cache()
//val df1 = spark.read.format("csv").load("dbfs:/FileStore/shared_uploads/zarius3@free.fr/credit.csv")
//val data  = spark.table("hmxpc13_di_v2_5_14_14_csv") 
...
// COMMAND ----------
  ...
  labelDf.printSchema()
  labelDf.select("creditability","label","features").show(10)
...
// COMMAND ----------
  ...
  predictionDf.select("creditability","label","features","rawPrediction","probability","prediction").show(10)
...
// we run creditDf on the pipeline, so split creditDf
  ...
  pipelinePredictionDf.select("creditability","label","features","rawPrediction","probability","prediction").show(10)
...
// COMMAND ----------
  ...
  cvPredictionDf.select("creditability","label","features","rawPrediction","probability","prediction").show(10)
...
// COMMAND ----------
 ...
 // save model 
//cvModel.write.overwrite().save("metadata/")
...
//val cvModelLoaded = CrossValidatorModel.load("/FileStore/fichier/")
val cvModelLoaded = cvModel
...
// COMMAND ----------
  ...
  df2.select("balance","credits").show()
